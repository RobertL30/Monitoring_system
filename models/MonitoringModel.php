...
class MonitoringModel {
    ...
    // Main public method - entry point with retry logic
    public function monitorDevice($device) {
        return $this->monitorDeviceWithRetries($device, 3);
    }

    // Retry logic with failure escalation
    public function monitorDeviceWithRetries($device, $maxRetries = 3) {
        $attempts = 0;
        $lastResult = null;

        while ($attempts < $maxRetries) {
            $attempts++;
            $result = $this->monitorDeviceSingle($device);
            $lastResult = $result;
            ...
            // If failed but not final attempt, wait briefly and retry
        ...
        $this->updateDeviceStatus($device, $lastResult);
        $this->logMonitoringHistory($device['id'], $lastResult);
    ...
    // Single monitoring attempt - no retries
    private function monitorDeviceSingle($device) {
    ...
    // Optimized ping - 2 pings instead of 4 (handles ARP + gets timing)
    private function pingDevice($ip) {
        $success_count = 0;
        $total_time = 0;
        $ping_count = 2; // Reduced from 4 to 2 - perfect for ARP + timing
